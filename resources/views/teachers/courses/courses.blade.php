    ...
    <div class="bg-white px-8">
        <div class="border-t border-gray-200 py-4">
            <div class="flex justify-between items-center">
                <div>
                    <h3 class="text-2xl font-semibold text-gray-700">Vos cours</h3>
                </div>
                <a href="/courses/create" class="inline-flex items-center px-4 py-2 border border-transparent rounded-md shadow-sm text-sm font-medium text-white bg-indigo-600 hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500">
                        <!-- Heroicon name: check -->
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        ...
                        <tr>
                            ...
                            <td class="flex justify-between px-6 py-4 whitespace-nowrap text-right text-sm font-medium" x-data="{}">
                                <a href="{{ url('/courses/'.$item->id.'/edit') }}" class="text-indigo-600 hover:text-indigo-900">Modifier</a>
                                <form method="post" action="courses/{{ $item->id }}" x-ref="form">
                                    @method('DELETE')
                                    @csrf
